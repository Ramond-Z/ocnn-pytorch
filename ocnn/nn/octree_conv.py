...
class OctreeConvBase:
    ...
    def check_and_init(self, data: torch.Tensor):
        ...
        assert check, 'The shape of input data is wrong. expected {}, got {} instead'.format(self.in_shape, data.shape)

        # Init the output data
        out = data.new_zeros(self.out_shape)
        return out

    def memory_efficient_gemm_forward(
        self,
        out: torch.Tensor,
        data: torch.Tensor,
    ...
    def memory_efficient_gemm_backward(
        self,
        out: torch.Tensor,
        grad: torch.Tensor,
        weights: torch.Tensor
    ...
    def memory_efficient_gemm_backward_weight(
        self,
        out: torch.Tensor,
        data: torch.Tensor,
        grad: torch.Tensor
    ...
    def forward_gemm(self, out, data, weights):
